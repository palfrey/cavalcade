...
async fn get_db_connection() -> Result<PgPool> {
    ...
    PgPool::connect(&database_url).await.map_err(|e| {
        ...
        ae.context(format!("Trying to connect to {}", database_url))
    })
}
...
async fn dump_dot(conn: &PgPool) {
    ...
    sqlx::query!("SELECT routing_key, queue._name as \"q_name!\", exchange._name as \"e_name!\" FROM bind JOIN queue ON queue.id = queue_id JOIN exchange ON exchange.id = exchange_id")        
        ...
        .unwrap()
        .into_iter()
        .for_each(|r| writeln!(graph_string, "E_{} -> Q_{}[label=\"{:?}\"]", r.e_name.replace(['-', '.', '@'], "_"), r.q_name.replace(['-', '.', '@'], "_"), r.routing_key).unwrap());
